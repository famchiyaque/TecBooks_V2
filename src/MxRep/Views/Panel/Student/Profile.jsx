...
import { useGetStudentProfile } from '@/MxRep/utils/hooks/student.hooks'
import Loader from '@/Global Components/Loader'
import { Button } from '@/components/ui/button'
import { Alert, AlertDescription } from '@/components/ui/alert'
import ProfileForm from '@/MxRep/Forms/Panels/Student/Profile'
import { AlertCircle, X } from 'lucide-react'

function Profile() {
  const navigate = useNavigate()
  const { user, isLoading, isInitialized } = useAuth()
  const { getStudentProfile, updateStudentProfile, profileIsLoading, error, profile } = useGetStudentProfile()
  ...
  useEffect(() => {
    ...
    console.log("[PROFILE VIEW] getting student profile for userId: ", user.userId)
    getStudentProfile(user.userId)
  }, [isInitialized, isLoading, user, getStudentProfile])
  ...
  const handleSave = async (data) => {
    ...
    const result = await updateStudentProfile(user.userId, data)
    setIsSaving(false)
