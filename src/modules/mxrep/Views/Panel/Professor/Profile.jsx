...
import { useGetProfessorProfile } from '@/MxRep/utils/hooks/professor.hooks'
import Loader from '@/components/global/Loader'
import { Button } from '@/components/ui/button'
import { Alert, AlertDescription } from '@/components/ui/alert'
import ProfileForm from '@/MxRep/Forms/Panels/Professor/Profile'
import { AlertCircle, X } from 'lucide-react'

function Profile() {
  const navigate = useNavigate()
  const { user, isLoading, isInitialized } = useAuth()
  const { getProfessorProfile, updateProfessorProfile, profileIsLoading, error, profile } = useGetProfessorProfile()
  ...
  useEffect(() => {
    ...
    console.log("[PROFILE VIEW] getting professor profile for userId: ", user.userId)
    getProfessorProfile(user.userId)
  }, [isInitialized, isLoading, user, getProfessorProfile])
  ...
  const handleSave = async (data) => {
    ...
    const result = await updateProfessorProfile(user.userId, data)
    setIsSaving(false)
