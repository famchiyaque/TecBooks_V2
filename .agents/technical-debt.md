...
- Employee labor category is inferred from the **name/title** in three places (`applyDerivedBase`, `Employee.js`, `cbm-to-game.mapper.js`). `employees` has `job_title` but no category column; upload even stores type/category into `job_title`. Inflows Costs per Unit now buckets titles itself (`sumAnnualSalariesByTitle`: MOD → direct, MOID → indirect, IM / Ingeniero / Gerente de Operaciones → engineering, everyone else → administrative) then divides annual `12 × monthly × headcount` by purchase orders. Cost Table still uses the older name parse (`operacion` leftovers → indirect, unknown → dropped). Add an employee category field and stop parsing titles.
- `computeWorkforceAnualSalaries` still divides by customer orders despite the name. Outflows no longer uses it (salaries come from `sumSalariesByCategoryPerYear`, same buckets as the Cost Table). Do not change the shared function until remaining callers of `computeProductionCost` are reviewed.
- `costCalculations.computeAdminExpenses` sums `services[].monthlyAmount` and inflates it, but never ×12, so the series is monthly. Callers left: `computeProductionCost` and `computeInvestment` (Working Capital / loan principal for Balance, Flujo, Net Flow Graph). Outflows now uses `src/utils/dashboard/computeAdminExpenses.js` (already ×12) for yearly services and year-0 working capital, so its loan principal can differ from Flujo/Balance until those tabs are reviewed. Inflows Unit Costs still ×12 locally in `toCostPerWorkOrder`. When Investment/Flujo are reviewed, annualize in the shared helper and remove that local ×12.
- Loan schedules (`computeAmortizationInterest`, `computeAmortizationSchedule`) assume origination on month 1 of project year 0: period 0 is January of `timeline.years[0]`, and Outflows amortization / interest / Loan Balance inherit that. A later start month is not an input. Do not shift the schedule until origination month is a real field.
