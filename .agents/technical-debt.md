...
- `computeAdminExpenses` sums `services[].monthlyAmount` and inflates it, but never ×12, so the series is monthly. Callers: `computeProductionCost` (Outflows Financial Expenses via `total`) and `computeInvestment` (Working Capital, loan principal → Outflows amortization, Balance long-term passives, Net Flow Graph). Outflows `calculateAdminExpenses` is the same monthly-sum bug with different inflation. Inflows Unit Costs now ×12 locally in `toCostPerWorkOrder` only. When Outflows/Investment are reviewed, annualize in the shared helper and remove that local ×12.
- Demand growth is read from Premisas **B33** (cell address, no label). `projectPurchaseOrders` annualizes YTD months / those months’ shares, averages with `demand.history` when present, and grows `base × (1 + demandGrowth)^k`. Year 0 stays the workbook actual. Needs `worker/migrations/0012_demand_growth.sql`. Projects uploaded before this must be re-uploaded for monthly actuals and history to persist (`fixed_amount` on monthly distribution + extra yearly-total rows).
